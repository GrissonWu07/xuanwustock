...
type ReplayTask = ReplaySnapshot["tasks"][number];
...
function drillTableTotal(table: { total?: number; items?: Array<Record<string, unknown>> } | undefined) {
  return Number(table?.total ?? table?.items?.length ?? 0);
}

function drillText(value: unknown, fallback = "--") {
...
function drillTable(
  ...
  mapCells: (item: Record<string, unknown>) => string[],
  emptyLabel: string,
  emptyMessage: string,
): TableSection {
  return {
    ...
    rows: (items ?? []).map((item, index) => {
      const code = drillText(item.stockCode, String(index));
      return {
        id: `${code}-${index}`,
        cells: mapCells(item),
        code,
        name: drillText(item.stockName, ""),
      };
    }),
    emptyLabel,
    emptyMessage,
...
function LiveQuantDrillTrendChart({ series }: { series: NonNullable<ReplayTask["lifecycleSeries"]> }) {
  if (!series.length) {
    return <>{t("暂无生命周期趋势")}</>;
  }
  const points = series.slice(-24);
  const statuses = [
    { key: "trialCount", label: "trial", color: "#7c8cff" },
    { key: "activeCount", label: "active", color: "#16a34a" },
    { key: "exitOnlyCount", label: "exit_only", color: "#f59e0b" },
    { key: "coolingCount", label: "cooling", color: "#64748b" },
    { key: "retiredCount", label: "retired", color: "#ef4444" },
  ] as const;
  const maxTotal = Math.max(
    1,
    ...points.map((point) => statuses.reduce((sum, status) => sum + Number(point[status.key] ?? 0), 0)),
  );

  return (
    <div className="stack" aria-label={t("生命周期趋势图")}>
      <div className="chip-row">
        {statuses.map((status) => (
          <span className="badge badge--neutral" key={status.key}>
            <span aria-hidden="true" style={{ display: "inline-block", width: 8, height: 8, borderRadius: 999, background: status.color }} />{" "}
            {status.label}
          </span>
        ))}
      </div>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, minHeight: 120, overflowX: "auto", paddingTop: 8 }}>
        {points.map((point) => {
          const total = statuses.reduce((sum, status) => sum + Number(point[status.key] ?? 0), 0);
          return (
            <div
              key={point.checkpointAt}
              title={`${point.checkpointAt} · ${statuses.map((status) => `${status.label} ${point[status.key]}`).join(" · ")}`}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 28 }}
            >
              <div
                aria-label={t("{v0} 生命周期状态总数 {v1}", { v0: point.checkpointAt, v1: total })}
                style={{
                  display: "flex",
                  flexDirection: "column-reverse",
                  width: 18,
                  height: Math.max(12, Math.round((total / maxTotal) * 92)),
                  overflow: "hidden",
                  borderRadius: 6,
              ...
              >
                {statuses.map((status) => {
                  const value = Number(point[status.key] ?? 0);
                  return value > 0 ? (
                    <span
                      key={status.key}
                      style={{
                        display: "block",
                        height: `${Math.max(8, (value / Math.max(1, total)) * 100)}%`,
      ...
      <div className="chip-row">
        {points.map((point) => (
          <span className="badge badge--neutral" key={point.checkpointAt}>
            {point.checkpointAt} · trial {point.trialCount} · active {point.activeCount} · exit_only {point.exitOnlyCount} · cooling {point.coolingCount} · retired {point.retiredCount}
          </span>
        ))}
      </div>
    </div>
...
function LiveQuantDrillLifecyclePanel({ task }: { task: ReplayTask | null }) {
  const isDrill = task?.runType === "live_quant_drill" || task?.mode === "live_quant_drill";
  if (!task || !isDrill) {
    return null;
  }
  const summary = task.lifecycleSummary ?? {
    initialQuantCount: 0,
  ...
  const summaryMetrics = [
    { label: t("初始量化股票"), value: summary.initialQuantCount },
    { label: t("历史候选事件"), value: summary.candidateEventCount },
    { label: t("自动入池"), value: summary.autoPromotedCount },
    { label: t("自动出池"), value: summary.autoExitedCount },
    { label: t("只出场管理"), value: summary.exitOnlyCount },
    { label: t("冷却"), value: summary.coolingCount },
    { label: t("已退出"), value: summary.retiredCount },
    { label: t("数据风险"), value: summary.dataWarningCount },
  ];
  const candidateEventsTable = drillTable(
    [t("时间"), t("股票"), t("来源"), t("评分"), t("状态变化"), t("原因")],
    task.candidateEventsTable?.items,
    (item) => [
      drillText(item.checkpointAt),
      `${drillText(item.stockCode)} ${drillText(item.stockName, "")}`.trim(),
      drillText(item.sourceType),
      drillNumber(item.candidateScore),
      drillText(item.statusChange),
      drillText(item.reason),
    ],
    t("暂无入池事件"),
    t("本次演练尚未生成历史候选事件。"),
  );
  const exitEventsTable = drillTable(
    [t("时间"), t("股票"), t("状态变化"), t("健康分"), t("原因")],
    task.exitEventsTable?.items,
    (item) => [
      drillText(item.checkpointAt),
      `${drillText(item.stockCode)} ${drillText(item.stockName, "")}`.trim(),
      `${drillText(item.fromStatus)} -> ${drillText(item.toStatus)}`,
      drillNumber(item.healthScore),
      drillText(item.reason),
    ],
    t("暂无出池或降级事件"),
    t("本次演练尚未触发生命周期降级或出池。"),
  );
  const finalStatesTable = drillTable(
    [t("股票"), t("最终状态"), t("清算盈亏"), t("状态变更"), t("原因")],
    task.finalStatesTable?.items,
    (item) => [
      `${drillText(item.stockCode)} ${drillText(item.stockName, "")}`.trim(),
      drillText(item.finalStatus),
      drillText(item.liquidationPnl ?? item.realizedPnl, "0"),
      drillNumber(item.stateChangeCount),
      drillText(item.latestReason),
    ],
    t("暂无最终状态"),
    t("本次演练尚未写入股票生命周期最终状态。"),
  );
  const dataRisksTable = drillTable(
    [t("股票"), t("域"), t("来源"), t("原因")],
    task.dataRisksTable?.items,
    (item) => [drillText(item.stockCode), drillText(item.domain), drillText(item.provider), drillText(item.reason)],
    t("暂无数据风险"),
    t("本次演练没有记录数据风险。"),
  );
  ...
  return (
    <div className="stack">
      <WorkbenchCard>
        <div className="replay-stock-scope__header">
          <div>
            <h2 className="section-card__title">{t("实时量化演练")}</h2>
            <p className="section-card__description">{t("展示本次演练中的自动入池、生命周期出池和数据风险结果。")}</p>
          </div>
          <span className="badge badge--accent">{task.typeLabel ? t("演练结果") : t("生命周期结果")}</span>
        </div>
        <div className="mini-metric-grid replay-task-metrics-grid">
          {summaryMetrics.map((metric) => (
            <div className="mini-metric replay-task-metric" key={metric.label}>
              ...
              <div className="mini-metric__value">{drillNumber(metric.value)}</div>
            </div>
          ))}
        </div>
        <div className="summary-list">
          <div className="summary-item summary-item--accent">
            <div className="summary-item__title">{t("生命周期趋势")}</div>
            <div className="summary-item__body">
              <LiveQuantDrillTrendChart series={task.lifecycleSeries ?? []} />
            </div>
          </div>
        </div>
      </WorkbenchCard>
      <div className="section-grid">
        <QuantTableSectionCard
          title={t("入池事件")}
          description={t("历史候选事件进入量化生命周期的记录。")}
          table={candidateEventsTable}
          emptyTitle={candidateEventsTable.emptyLabel ?? t("暂无入池事件")}
          emptyDescription={candidateEventsTable.emptyMessage ?? t("本次演练尚未生成历史候选事件。")}
          tableLayout="auto"
        />
        <QuantTableSectionCard
          title={t("出池与降级事件")}
          description={t("自动出池、冷却和只出场管理的状态变化。")}
          table={exitEventsTable}
          emptyTitle={exitEventsTable.emptyLabel ?? t("暂无出池或降级事件")}
          emptyDescription={exitEventsTable.emptyMessage ?? t("本次演练尚未触发生命周期降级或出池。")}
          tableLayout="auto"
        />
      </div>
      <div className="section-grid">
        <QuantTableSectionCard
          title={t("股票最终状态")}
          description={t("演练结束时每只量化股票的生命周期状态。")}
          table={finalStatesTable}
          emptyTitle={finalStatesTable.emptyLabel ?? t("暂无最终状态")}
          emptyDescription={finalStatesTable.emptyMessage ?? t("本次演练尚未写入股票生命周期最终状态。")}
          tableLayout="auto"
        />
        <QuantTableSectionCard
          title={t("数据风险")}
          description={t("历史候选、行情或公司行为数据不可用时的风险记录。")}
          table={dataRisksTable}
          emptyTitle={dataRisksTable.emptyLabel ?? t("暂无数据风险")}
          emptyDescription={dataRisksTable.emptyMessage ?? t("本次演练没有记录数据风险。")}
          tableLayout="auto"
        />
      </div>
      <div className="summary-item__body">
        {t("明细总数：入池 {v0}，出池与降级 {v1}，最终状态 {v2}，数据风险 {v3}", {
          v0: drillTableTotal(task.candidateEventsTable),
          v1: drillTableTotal(task.exitEventsTable),
          v2: drillTableTotal(task.finalStatesTable),
          v3: drillTableTotal(task.dataRisksTable),
        })}
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className="section-grid section-grid--sidebar">
        ...
        <div className="stack">
          ...
          <LiveQuantDrillLifecyclePanel task={selectedTask} />
