...
export function HistoryPage({ client }: HistoryPageProps) {
  ...
  if (resource.status === "loading" && !resource.data) {
    return <PageLoadingState title={t("历史记录加载中")} description={t("正在读取分析记录、回放结果和操作轨迹。")} />;
  }
  ...
  if (resource.status === "error" && !resource.data) {
    return (
      <PageErrorState
        title={t("历史记录加载失败")}
        description={resource.error ?? t("无法加载历史记录数据，请稍后重试。")}
        actionLabel={t("重新加载")}
  ...
  if (!snapshot) {
    return <PageEmptyState title={t("历史记录暂无数据")} description={t("后台尚未返回历史记录快照。")} actionLabel={t("刷新")} onAction={resource.refresh} />;
  }
  ...
  return (
    <div>
      <PageHeader eyebrow="History" title={t("历史记录")} description={t("分析记录、历史回放、工作流轨迹都会在这里统一沉淀，方便后续复盘。")} />
      <div className="stack">
        <WorkbenchCard>
          <h2 className="section-card__title">{t("快照概览")}</h2>
          <p className="section-card__description">
            {t("历史记录、回放概况和时间线由同一份快照提供，刷新后会统一同步最新的分析轨迹。")}</p>
          <div className="mini-metric-grid">
            ...
            <div className="mini-metric">
              <div className="mini-metric__label">{t("分析记录")}</div>
              <div className="mini-metric__value">{snapshot.records.rows.length}</div>
            </div>
            <div className="mini-metric">
              <div className="mini-metric__label">{t("工作流轨迹")}</div>
        ...
        <div className="section-grid">
          <WorkbenchCard>
            <div className="toolbar">
              <div>
                <h2 className="section-card__title">{t("分析记录")}</h2>
                <p className="section-card__description" style={{ marginBottom: 0 }}>
                  {t("所有单股分析、量化模拟和回放结果都会保存在这里，便于统一回看。")}</p>
              </div>
            ...
            {!hasRecords ? (
              <SectionEmptyState title={t("分析记录暂无数据")} description={t("当前没有可展示的分析记录，稍后可点击重新整理或等待新的结果写入。")} />
            ) : null}
            ...
            <div className="toolbar toolbar--compact" style={{ marginTop: 12 }}>
              <span className="toolbar__status">{t("DB筛选")}{totalRows} {t("条 · 第")}{currentPage} / {totalPages} {t("页")}</span>
              <span className="toolbar__spacer" />
          ...
          <WorkbenchCard>
            <h2 className="section-card__title">{t("最近回放")}</h2>
          ...
          <WorkbenchCard>
            <h2 className="section-card__title">{t("工作流轨迹")}</h2>
            {snapshot.timeline.length === 0 ? (
              <SectionEmptyState title={t("工作流轨迹暂无数据")} description={t("当分析、回放或整理动作写回后，这里会继续展示时间线。")} />
            ) : (
