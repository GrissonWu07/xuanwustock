...
3. 允许系统根据候选质量、趋势健康度、执行反馈和组合风险，自动决定股票是进入量化观察、正式扫描、只出场管理、冷却，还是退出
...
生命周期事件是股票在量化管理域中发生状态变更的留痕，例如“量化观察纳入”“进入冷却”“退出自动量化”。
...
9. 自动开启后的可视反馈必须明显可见：
   - 新进入量化观察的股票显示 `trial` badge 和“量化观察”中文标签
...
| `/discover` | `纳入量化观察` 行按钮 | 每行操作列 | 确认弹窗 -> 单行提交 -> toast -> 行状态更新 |
| `/discover` | `一键批量纳入量化观察` | 表格工具栏 | 选中多行 -> 确认弹窗 -> 批量提交 -> 部分失败逐行回显 |
...
1. aggressive 允许更多量化观察标的与更慢退场，但仍受 `exit_only` 和组合风控约束
