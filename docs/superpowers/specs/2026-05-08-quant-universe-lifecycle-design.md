...
3. 历史回放启动时冻结 `quant_enabled=1` 范围
...
3. 允许系统根据候选质量、趋势健康度、执行反馈和组合风险，自动决定股票是进入试运行、正式扫描、只出场管理、冷却，还是退出
...
生命周期事件是股票在量化管理域中发生状态变更的留痕，例如“试运行纳入”“进入冷却”“退出自动量化”。
...
1. 启动任务时冻结当前 `quant_enabled=1` 范围
2. 不在回放过程中模拟生命周期动态变化
...
3. 顶部配置面板中新增一组 `量化池自动管理` 控件，位置固定在“策略/profile 选择器”右侧、“自动执行”开关左侧
4. 该组控件必须包含：
   - `生命周期管理` 总开关
   - `自动入池模式` 下拉
   - `自动出池` 开关
...
2. `生命周期管理` 总开关
   - 关闭后：
     - 不再执行自动入池
     - 不再执行基于 `health_score` 的自动降级、冷却、退出、自动恢复
     - 已有生命周期状态、健康度、原因、候选分仍然展示
   - 开启后：
     - 恢复候选事件聚合、自动状态推进和自动恢复链路
   - 切换总开关需要二次确认
   - 切换成功后，顶部摘要文案必须立即更新
3. `自动入池模式` 下拉
   - 可选值：
   ...
   - 修改后只影响后续新产生的候选事件
   - 不回溯改写已存在股票状态
   - 修改成功后应 toast 提示“后续候选将按新模式处理”
4. `自动出池` 开关
   - 关闭后：
     - 仍计算 `health_score`
     - 仍展示 weakening/downtrend
     - 但不自动执行 `trial/active/exit_only -> cooling/retired`
   - 开启后恢复自动出池
   - 切换需要二次确认
...
| `/discover` | `纳入 trial` 行按钮 | 每行操作列 | 确认弹窗 -> 单行提交 -> toast -> 行状态更新 |
| `/discover` | `一键批量纳入 trial` | 表格工具栏 | 选中多行 -> 确认弹窗 -> 批量提交 -> 部分失败逐行回显 |
...
| `/live-sim` | `LifecycleMasterSwitch` | 顶部配置面板 | 控制生命周期自动管理总开关 |
| `/live-sim` | `AutoEntryModeSelect` | 顶部配置面板 | 切换 `manual_only / confirm_first / auto_trial` |
| `/live-sim` | `AutoExitSwitch` | 顶部配置面板 | 控制自动出池开启/关闭 |
| `/live-sim` | `LifecycleSummaryBadgeGroup` | 顶部配置面板 | 展示当前运行口径摘要 |
...
1. aggressive 允许更多试运行与更慢退场，但仍受 `exit_only` 和组合风控约束
